...
def benchmark_attention(BATCH_SIZE, NUM_HEADS, SEQ_LEN, HEAD_DIM, causal, dtype=torch.float16, num_warmup=10, num_runs=100):
    """Benchmark normal attention vs flash attention"""
    ...
    for _ in range(num_runs):
        ...
        V.grad = None
    torch.cuda.synchronize()
    normal_bwd_time = (time.time() - start_time) / num_runs * 1000  # Convert to ms
    normal_total_time = normal_fwd_time + normal_bwd_time
    ...
    for _ in range(num_runs):
        ...
        V.grad = None
    torch.cuda.synchronize()
    flash_bwd_time = (time.time() - start_time) / num_runs * 1000  # Convert to ms
    flash_total_time = flash_fwd_time + flash_bwd_time
    ...
    print(f"  Backward: {normal_bwd_time:.3f} ms")
    print(f"  Total:    {normal_total_time:.3f} ms")
    
    print(f"\nFlash Attention (Triton):")
    print(f"  Forward:  {flash_fwd_time:.3f} ms")
    print(f"  Backward: {flash_bwd_time:.3f} ms")
    print(f"  Total:    {flash_total_time:.3f} ms")
    
    print(f"\nSpeedup:")
    print(f"  Forward:  {normal_fwd_time / flash_fwd_time:.2f}x")
    print(f"  Backward: {normal_bwd_time / flash_bwd_time:.2f}x")
    print(f"  Total:    {normal_total_time / flash_total_time:.2f}x")
    print(f"{'='*80}\n")


def test_op(BATCH_SIZE, NUM_HEADS, SEQ_LEN, HEAD_DIM, causal, dtype=torch.float16):
    """Test correctness of flash attention implementation"""
    Q = (
        torch.empty(
...
if __name__ == "__main__":
    ...
    with open("flash_attention_output.txt", "w") as f:
        ...
        # You can also run your benchmark function here if needed
        # benchmark_attention(...)